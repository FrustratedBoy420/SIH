# PS26167 - Official Problem Statement (Authoritative Text)
> **Source of truth.** Every line below is extracted verbatim from
> `sih_2026_problem_statements.json`, the official SIH 2026 problem-statement
> dump, record ID `26167`. Only three cosmetic changes were made: HTML entities
> were decoded (`&#8226;` to a bullet), `<br>` tags became line breaks, and the
> mojibake character `�` - a bullet that was corrupted during the original
> PDF-to-JSON export - was replaced with a dash.
>
> **Cite this file, not a third-party website.** Unofficial mirrors of the SIH
> problem statements exist and are widely quoted. They currently agree with this
> text, but they are a dependency you do not need. When any other document in
> this repository states 'the PS requires X', it is quoting this file.
...
## Dataset Link field (verbatim)

Training / Fine-Tuning Dataset BigEarthNet.txt â€” primary dataset for remote-sensing adaptation using co-registered Sentinel-1 SAR, Sentinel-2 multispectral imagery, and diverse text annotations. Link: https://arxiv.org/abs/2603.29630. All datasets are available online open source.

 Public Evaluation Benchmarks

 - VRSBench â€” for r

> **Defect in the source data - the `Dataset Link` field above is truncated.**
> It ends mid-word at `VRSBench - for r`. This is not a transcription error on our
> side. Across all 226 records in the JSON, the longest `Dataset Link` value is
> exactly 355 characters, which is the length of this one - the export that produced
> the file cut every dataset field at that limit. The consequence is concrete: the
> official guidance for **VRSBench, RSVQA and CDVQA** (their links and any usage
> notes) was cut off and is *not* available to us from this file. Only the
> BigEarthNet.txt guidance survived intact. Recover the full field from the official
> SIH portal listing before finalising the data plan.

---

## Two things to notice in the text above

```mermaid
flowchart TD
    PS["PS26167 official text"] --> N1["1. The judging table is MISSING<br/>literal placeholder:<br/>'Add Evaluation/Judging Criteria table here'"]
    PS --> N2["2. Only the OBSERVABLE execution trace<br/>is evaluated - internal reasoning<br/>is neither required nor evaluated"]

    N1 --> W1["Weights are UNKNOWN.<br/>Do not over-invest in one capability.<br/>Balance across all five mandatory ones."]
    N2 --> W2["DO show: task, tools, parameters, outputs.<br/>DO NOT show: fake chain-of-thought.<br/>Most teams will get this backwards."]

    NOTE["This placeholder appears in 8 of the<br/>155 software statements - a systemic<br/>editorial failure, not specific to 26167."]
    N1 -.-> NOTE

    style N1 fill:#f8d7da,stroke:#b02a37,color:#000
    style N2 fill:#cfe2ff,stroke:#084298,color:#000
```


**1. The judging-criteria table is missing.** Under *Evaluation/Judging Criteria* the
statement contains the literal editorial placeholder `Add 'Evaluation/Judging Criteria'
table here`. The table was never filled in before publication. The PS therefore tells us
*which* datasets are used for scoring and that "scores will be normalised before combining
different metrics", but never states the weights. We do not know whether VQA accuracy,
grounding IoU, change F1 or routing accuracy carries the most marks. Plan for balance
across all five mandatory capabilities rather than optimising one.

**2. Only the observable execution trace is scored.** The statement says the controller
"may perform internal task planning; however, only the observable execution trace ...
will be evaluated. Internal reasoning text is neither required nor evaluated." This is an
explicit instruction about what to display. See the chain-of-thought section of the
analysis document.
