...
export default function Refusal({ optical }: { optical?: LoadedRaster }) {
  ...
  return (
    <section className="border-t border-ink bg-nir-bg/50 px-5 py-20 sm:px-10" data-testid="refusal-replay" aria-labelledby="refusal-h">
      <div ref={ref} className="grid gap-12 lg:grid-cols-[minmax(0,5fr)_minmax(0,6fr)]">
        <div>
          <p className="label mb-6">Refuse rather than guess</p>
          <h2 id="refusal-h" className="t-hero">It <span className="text-nir">refuses.</span></h2>
          <p className="mt-8 max-w-[460px] text-[17px] leading-[1.55] text-ink-2">
        ...
        <div className="border border-ink bg-surface p-5" key={key}>
          {inView && data && (
            <>
              <div className="flex flex-wrap items-center gap-2">
                <span className="mono border border-rule px-2 py-0.5 text-[11.5px]">1 raster · optical · {data.manifest.rasters[0]?.crs}</span>
              ...
              <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.6, duration: 0.3 }} className="mt-4 border-l-[3px] border-nir bg-nir-bg/60 px-4 py-3">
          ...
          <button type="button" onClick={() => setNonce((n) => n + 1)} className="mono mt-4 text-[12px] text-accent hover:underline">↻ replay</button>
        </div>
      </div>
    </section>
  )
