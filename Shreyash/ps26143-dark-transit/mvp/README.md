...
python3 -m darktransit.cli selftest        # the acceptance criteria of PRD section 19
...
**Real:** everything that reads them. The speckle filter, the adaptive threshold, connected components, the six-feature look-alike discriminator, PCA geometry, RK4 advection with leeway and a diffusive random walk, the per-particle forcing ensemble, the density-quantile origin region, per-vessel cadence baselines, the reachable-set ellipse and its overlap with the region, CA-CFAR with a stated `P_fa`, target-to-AIS matching, the weighted score, all five gates and the dossier.
...
Not implemented, and logged as a degradation when the run starts: the fine-tuned U-Net of DT-3. The classical path stands alone. See PRD §22 for the full traceability table.
...
darktransit/
  ...
  tests.py         the acceptance criteria, executable
